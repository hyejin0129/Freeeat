...
// 유경 관리자 기능 시작
	public ArrayList<Place> placeSelectList(String sort){ // 전체 리스트 조회
        ArrayList<Place> list = null;
         Connection conn = getConnection();  
         if(sort.equals("A")) list = new PlaceDao().placeAllList(conn);
         else list = new PlaceDao().placeSelectList(conn,sort);
         close(conn);
         return list;
      }
      ...
      public int placeDelete(int pno) {
         ...
         // 메뉴는 상태값 없어서 따로 삭제 안함 > 상세페이지에서 보여줘야하기 때문에
         int result2 = 1; // 사진없으면 dao 가서 변경 안해도 되니까 
        
         if(placePhotoSelect(pno).getPlaceNo() != 0) {   
            result2 = new PlaceDao().placePhotoDelete(conn,pno);             
         }
      ...
      public int placeCheckDelete(String[] clist){
         ...
         for(int i = 0; i < clist.length; i++) { // 배열은 length, 하나의 식당+메뉴+사진 세트로 지워야함
            
            int pno = Integer.parseInt(clist[i]);
            result *= placeDelete(pno);      
         }
         close(conn);
         return result;
      }
    // 식당 수정
      public int placeUpdate(Place place, ArrayList<PlaceMenu> list, PlacePhoto photo){
         ...
         if(photo != null){ // 파일 추가하면 
              if(photo.getPlacePhotoNo() != 0) {
      ...
      public int requestUpdate(int reqNo, int pno) {
         Connection conn = getConnection();
         int result1 = new PlaceDao().requestUpdate(conn, reqNo);
         int result2 = 0;
         
         if(result1 > 0) {
            result2 = placeDelete(pno);
         }
